...
export default class Board {
  ...
  _handleLoadMoreButtonClick() {
    ...
    const newRenderedTaskCount = Math.min(taskCount, this._renderedTaskCount + TASKS_COUNT_PER_STEP);
  ...
  _renderTaskList() {
    const taskCount = this._getTasks().length;
    const tasks = this._getTasks().slice(0, Math.min(taskCount, TASKS_COUNT_PER_STEP));

    this._renderTasks(tasks);
    if (taskCount > TASKS_COUNT_PER_STEP) {
